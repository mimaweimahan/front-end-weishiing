<template>
	<view class="index">
		...
		<view class="content">
			<view class="title">{{$t('withdrawal.txt25')}}</view>
			<view class="dataView">
				<view class="data" @click="show = true">
					<view class="text">{{$t('withdrawal.txt26')}}</view>
					<view class="right">
						<text>{{coin}}</text>
			...
			<view class="amount">
				<view class="remainder">
					{{$t('withdrawal.txt27')}}: {{userInfo.money}} 
				</view>
				<view class="other">
		...
		<!-- range-key为cateName -->
		<u-picker range-key="address" mode="selector" v-model="show" :range="selector" @confirm="confirm"></u-picker>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			confirm(e) {
				let index = e[0]
				this.coin = this.selector[index].address
				this.addressId = this.selector[index].id
			},
			getUserAddressList() {
				this.$http.request('GET', this.$apis.getUserAddressList).then(res => {
					if (res.data.code == 1) {
